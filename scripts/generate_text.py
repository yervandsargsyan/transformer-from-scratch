...
def generate_text(model, tokenizer, seed_text="<bos>", max_len=400, device="cpu", temperature=1.0, top_p=0.9):
    """
    ...
    for _ in range(max_len):
        ...
        # Keep only tokens with cumulative prob <= top_p
        mask = cumulative_probs <= top_p
        if not mask.any():  # at least keep one token
            mask[0] = True
